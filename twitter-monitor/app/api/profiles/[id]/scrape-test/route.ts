...
// POST trigger scraping for a profile (using simple scraper)
export async function POST(
  ...
  try {
    ...
    // Scrape tweets using simple scraper
    const tweets = await simpleScraper.scrapeProfile(profile.username);
    ...
    return NextResponse.json({
      ...
      message: savedCount > 0 
        ? `Successfully scraped ${savedCount} tweets` 
        : 'No new tweets found (or using mock data - configure GhostFetch for real scraping)',
    });
  } catch (error: any) {
